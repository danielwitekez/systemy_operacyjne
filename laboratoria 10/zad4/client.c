...
void send_file(const char* file_path, int client_socket) {
    FILE *file;
    ...
    // Sending an empty packet as an indicator of the end of the file
    if (sendto(client_socket, "", 0, 0, (struct sockaddr*)&server_addr, sizeof(server_addr)) == -1) {
        perror("sendto");
        exit(EXIT_FAILURE);
    }
